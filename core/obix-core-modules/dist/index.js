...
const RELATIVE = /^(\.\.?)(\/|\\|$)/;
function encodePathDelimiters(s) {
...
export function createModuleResolver(hooks = {}) {
    const doImport = hooks.importImpl ?? ((s) => import(s));
    const doResolve = hooks.resolveImpl ?? defaultResolve;
    function resolveModule(specifier, parentURL) {
        ...
        if (r)
            return { input: specifier, parentURL, kind: "url", resolved: r, via: "import.meta.resolve" };
        return { input: specifier, parentURL, kind: "bare", resolved: specifier, via: "passthrough" };
    }
    async function loadModule(specifier, parentURL) {
        const resolved = resolveModule(specifier, parentURL);
        try {
            const namespace = (await doImport(resolved.resolved));
    ...
    function resolveAsset(assetSpecifier, ownerURL) {
    ...
    function validatePackageSet(manifests, opts) {
        ...
        for (const m of manifests) {
            ...
            if (!byName.has(m.name))
                byName.set(m.name, new Set());
            byName.get(m.name).add(m.version);
        }
        for (const [name, versions] of byName) {
            if (!name.startsWith(opts.familyPrefix))
                continue;
            if (versions.size > 1) {
                problems.push({
        ...
        for (const m of manifests) {
            if (!m?.name?.startsWith(opts.familyPrefix))
                continue;
            for (const [dep, range] of Object.entries(m.dependencies ?? {})) {
                if (!dep.startsWith(opts.familyPrefix))
                    continue;
...
function classifyLoadError(err, specifier, parentURL) {
    const e = err;
    const msg = e?.message ?? String(err);
    let code = "module/load";
    let reason = msg;
    let remediation;
...
export const modules = createModuleResolver();
